...
import type { ExtractedContent, SearchOptions, SearchResponse } from "../types.js";
...
let cachedConfig: { exaApiKey?: unknown } | null = null;
let warnedMonth: string | null = null;

function loadConfig(): { exaApiKey?: unknown } {
    ...
    try {
        cachedConfig = JSON.parse(raw) as { exaApiKey?: unknown };
        return cachedConfig;
    } catch (err) {
...
function mapResults(results: ExaSearchResponse["results"] | ExaAnswerResponse["citations"]): SearchResponse["results"] {
    ...
    for (let i = 0; i < results.length; i++) {
        const item = results[i];
        if (!item?.url) continue;
        mapped.push({ title: item.title || `Source ${i + 1}`, url: item.url, snippet: "" });
    }
    return mapped;
}
...
/** Check if Exa is available (API key or MCP) */
export function isExaAvailable(): boolean {
    if (hasExaApiKey()) {
        const usage = readUsage();
        return usage.count < MONTHLY_LIMIT;
    }
    return true; // MCP fallback always available
}
...
/**
 * Search using Exa.
 * Falls back to Exa MCP if no API key configured.
 */
