import { type Message, complete, getModel } from "@mariozechner/pi-ai";
...
async function resolveSummaryModel(
...
): Promise<{ model: AnyModel; apiKey: string; headers?: Record<string, string> }> {
    ...
    const preferredModels = getSummaryModels();
    ...
    throw new Error(
        `No API key available for summary models: ${preferredModels.map((c) => `${c.provider}/${c.id}`).join(", ")}`,
    );
}
