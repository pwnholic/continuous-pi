 ...
 * ──────────────────────────────────────────────
 * Extracts readable content from Next.js React
 * Server Components (RSC) flight payloads.
 *
 * RSC pages embed their content in
 * `<script>self.__next_f.push([...])</script>`
 * tags as JSON.  This parser walks the node tree
 * to produce clean markdown.
 *
 * @module extractors/rsc
 */

import type { IRSCExtractResult } from "../types/content.js";

// ── Public API ─────────────────────────────────

/**
 * Attempt to extract content from a Next.js RSC page.
 *
 * Returns `null` when the HTML does not contain RSC
 * flight data (i.e. there are no `__next_f.push` calls).
 *
 * @param html - The raw HTML of the page.
 * @returns The extracted title and markdown content, or `null`.
 */
export function extractRSCContent(html: string): IRSCExtractResult | null {
    if (!html.includes("self.__next_f.push")) {
        return null;
    }

    // Parse all RSC chunks into a map (chunk ID → payload string).
    ...
    for (const match of html.matchAll(scriptRegex)) {
        ...
        // Each line inside a chunk is "id:payload"
    ...
    // Extract title from standard <title> tag
    const titleMatch = html.match(/<title[^>]*>([^<]+)<\/title>/);
    const title = titleMatch?.[1]?.split("|")[0]?.trim() ?? "";

    // Parse cache for chunk ID → parsed JSON
    ...
    // Walks the RSC node tree, producing markdown text
    type Node = unknown;
    const visitedRefs = new Set<string>();

    function extractNode(
        node: Node,
        ctx: { inTable: boolean; inCode: boolean } = {
            inTable: false,
            inCode: false,
        },
    ): string {
        ...
        if (typeof node === "string") {
            ...
            // Skip RSC artifacts but preserve content in code blocks
    ...
    const contentParts: Array<{ order: number; text: string }> = [];
